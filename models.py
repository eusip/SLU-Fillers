...
class MaskedLM(BaseTransformer):
    ...
    def validation_epoch_end(self, outputs: dict) -> dict:
        ...
        if self.hparams.do_train and not self.hparams.do_validate:
        ...
        if self.hparams.do_validate:
            val_results = os.path.join(val_folder, "no_train_val_results.txt")
            Path(val_folder).mkdir(parents=True, exist_ok=True)
        
            if self.current_epoch == 0:
                with open(val_results, "w") as writer:
                    writer.write("***** Seed - {} *****\n".format(self.hparams.seed))
                    for key in sorted(logs.keys()):
                        writer.write("%s = %s\n" % (key, str(logs[key]. numpy())))
...
class ConfPrediction(BaseTransformer):
...
class ConfPredictionFT(BaseTransformer):
    """An instance of the `BertForSequenceClassification` model with an MLP classifcation layer."""
    def __init__(self, hparams):
        if type(hparams) == dict:
            hparams = Namespace(**hparams)
